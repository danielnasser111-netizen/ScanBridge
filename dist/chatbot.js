(() => {
  ...
  const css = `
    ...
    .milo-launch { position:fixed; right:16px; bottom:14px; z-index:30; display:grid; grid-template-columns:68px auto; gap:.35rem; align-items:center; padding:0; border:0; color:var(--ink); background:transparent; cursor:pointer; font-family:"DM Sans",sans-serif; text-align:left; }
    ...
    .milo-panel { position:fixed; right:22px; bottom:90px; z-index:31; display:flex; flex-direction:column; width:min(390px,calc(100% - 28px)); height:min(590px,calc(100vh - 112px)); overflow:hidden; border:1px solid var(--line); border-radius:18px; background:var(--paper); box-shadow:0 24px 70px rgba(0,0,0,.25); font-family:"DM Sans",sans-serif; }.milo-panel[hidden] { display:none; }
    ...
    @media(max-width:600px){.milo-launch{right:10px;bottom:8px;grid-template-columns:62px}.milo-companion{width:62px;height:70px}.milo-bubble{display:none}.milo-orbit{width:62px;height:62px}.milo-panel{right:14px;bottom:76px;height:min(570px,calc(100vh - 92px))}html[dir="rtl"] .milo-launch{left:10px;right:auto}html[dir="rtl"] .milo-panel{left:14px;right:auto}}
